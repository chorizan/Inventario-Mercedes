...
export function RegistrarProductos({ onClose, dataSelect, accion }) {
  ...
  const codigoBarrasValue = watch("codigobarras");

  useEffect(() => {
    async function renderBarcode() {
      try {
        if (!svgRef.current) return;
        const code = String(codigoBarrasValue || "");
        if (!code) {
        ...
        JsBarcode(svgEl, code, { format: "CODE128", displayValue: true, fontSize: 14, height: 50 });
      } catch (error) {
          ...
          <section className="seccion2">
            <article>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginTop:6}}>
                <div ref={svgRef} id="barcodePreview" />
                <button type="button" onClick={() => {
                  ...
                  const html = `
                    <html>
                      ...
                      <body>
                        <h3>${dataSelect.descripcion || ''}</h3>
                        <div>${svgRef.current ? svgRef.current.innerHTML : ''}</div>
                        <div style="margin-top:8px;">Codigo: ${codigoBarrasValue || ''}</div>
                      </body>
                    </html>
                  `;
                  popup.document.open();
